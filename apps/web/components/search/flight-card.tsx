...
export function FlightCard({ flight }: { flight: FlightResult }) {
  return (
    <article className="rounded border border-slate-200 bg-white p-4 shadow-sm transition hover:border-brand-200 hover:shadow-soft">
      <div className="grid gap-5 lg:grid-cols-[180px_1fr_210px] lg:items-center">
        <div>
        ...
        <div>
          <div className="grid grid-cols-[72px_1fr_72px] items-center gap-3">
            ...
            <div className="text-center">
              <p className="mb-2 inline-flex items-center gap-1 rounded bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                <Timer size={13} /> {formatDuration(flight.durationMin)}
              </p>
              <div className="relative h-px bg-slate-300">
                <span className="absolute left-1/2 top-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded bg-brand-600" />
              </div>
        ...
        <div className="rounded bg-slate-50 p-4 text-left lg:text-right">
          <p className="text-xs text-slate-500">Giá khách thanh toán</p>
          <p className="text-2xl font-bold text-coral">{currency.format(flight.displayPriceVND)}</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">Đã gồm thuế, phí và hoa hồng {currency.format(flight.markupVND)}</p>
